...
public class MossBlock extends Block
{
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k)
    {
        ...
        if (l >= 3)
        {
            return AxisAlignedBB.getBoundingBox((double)i + minX, (double)j + minY, (double)k + minZ, (double)i + maxX, (float)j + 0.5F, (double)k + maxZ);
        }
        else
        {
            return null;
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        if (random.nextInt(6) == 0 && world.getBlockLightValue(i, j, k) <= 12)
        {
