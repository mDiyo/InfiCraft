...
public class WorldGenSlimePools extends WorldGenerator implements IWorldGenerator
{
    ...
    public void generateSlimePool(World world, Random rand, int chunkX, int chunkZ) 
	{
		if(rand.nextInt(PHSlime.slimePoolRarity) == 0)
		{
			int hi = rand.nextInt(PHSlime.slimePoolHeight);
		...
		if (biome.temperature >= 0.8f && biome.rainfall >= 0.9f)
		{
			if(rand.nextInt(PHSlime.slimePoolRarity/4+1) == 0) 
					{
				int hi = rand.nextInt(PHSlime.slimePoolHeight * 3);
		        int randX = chunkX + rand.nextInt(16);
		        int randZ = chunkZ + rand.nextInt(16);
		        this.generate(world, rand, randX, hi, randZ);
			}
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) //WorldGenerator version
    {
        par3 -= 8;
                ...
                for (var8 = 0; var8 < 16; ++var8)
                {
                    for (var32 = 0; var32 < 16; ++var32)
                    {
                        for (var10 = 0; var10 < 8; ++var10)
                        {
                            ...
                            {
                                par1World.setBlock(par3 + var8, par4 + var10, par5 + var32, OrizonSlime.slimeGelBlock.blockID);
                            }
                        }
                    }
                }
