...
{
    public FloraCropBlock(int i, int j)
    {
        super(i, j);
    ...
    public ArrayList getBlockDropped(World world, int i, int j, int k, int l, int i1)
    {
        ...
        if (l == 3)
        {
            arraylist.add(new ItemStack(FloraCrops.food, 1, 0));
        }
        for (int j1 = 0; j1 < 3 + i1; j1++)
        {
            if (world.rand.nextInt(10) <= l)
            {
                arraylist.add(new ItemStack(FloraCrops.barleySeed));
            }
        }
