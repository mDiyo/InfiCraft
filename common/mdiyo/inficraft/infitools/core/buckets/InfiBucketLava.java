        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
            	...
            	if (world.getBlockMaterial(i, j, k) == Material.water && world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(Items.lavaBucketEmpty);
                }
            	if ((bID == Block.lavaMoving.blockID || bID == Block.lavaStill.blockID)
                		&& world.getBlockMetadata(i, j, k) == 0)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(Items.lavaBucketLava);
                }
            	if (bID == Block.sand.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(Items.lavaBucketGlass);
                }
            	if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(Items.lavaBucketCobblestone);
                }
            }
            else
            {
