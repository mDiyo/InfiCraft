...
public class InfiBucketCactus extends Item
{
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
                		...
                		&& world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(Items.cactusBucketWater);
                }
                if (bID == Block.sand.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(Items.cactusBucketSand);
                }
                if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(Items.cactusBucketGravel);
                }
            }
            else
            {
                if (isFull < 0)
                {
                    return new ItemStack(Items.cactusBucketEmpty);
                }
        ...
        {
            return new ItemStack(Items.cactusBucketMilk);
        }
        return itemstack;
    }
