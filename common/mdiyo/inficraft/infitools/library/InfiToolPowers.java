...
public class InfiToolPowers
{
    ...
    public static void slimePower(int x, int y, int z, World world)
    {
        ...
        if (random.nextInt(100) + 1 <= 5)
        {
            ...
            try
            {
            	Method method = entityslime.getClass().getMethod("setSlimeSize", new Class[]{int.class});
            	method.setAccessible(true);
            	method.invoke(entityslime, 1);
            }
            catch (Exception e)
            {
            	try
            	{
            		Method method = entityslime.getClass().getMethod("a", new Class[]{int.class});
                	method.setAccessible(true);
                	method.invoke(entityslime, 1);
            	}
            	catch (Exception ex)
