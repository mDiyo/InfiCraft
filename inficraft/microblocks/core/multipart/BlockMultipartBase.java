...
public abstract class BlockMultipartBase extends BlockContainer {
	...
	public final boolean canHarvestBlock(EntityPlayer ply, int meta) {
		...
		//return canHarvestPart(ply, meta);
		return true;
	}
	...
	public boolean removeBlockByPlayer(World w, EntityPlayer ply, int x, int y, int z) {
		removeBlockByPlayerStatic(w, ply, x, y, z);
		return true;
	}
	
	public static void removeBlockByPlayerStatic(World w, EntityPlayer ply, int x, int y, int z) {
		...
		if(ply.worldObj.isRemote) {
			...
			return; // on the client, do nothing (let the server handle multipart breaking)
	...
	public ArrayList<ItemStack> getPartDrops(World world, int x, int y, int z, int metadata, int fortune, int subhit) {
	...
	public void renderInvBlock(RenderBlocks render, int meta) {
		...
		String tex = wrappedGetTextureFile();
		if(!tex.equals("/terrain.png"))
			ForgeHooksClient.bindTexture(tex, 0);
		render.renderBlockAsItem(this, meta, 0);
		if(!tex.equals("/terrain.png"))
