...
public class MicroblockPlacementHighlightHandler {
	...
	public void onHighlight(DrawBlockHighlightEvent evt) {
		...
		{
			PartType type = MicroblockSystem.parts.get(ItemMicroblock.getPartID(holding));
			if(type != null)
			{
				...
				if(ok)
				{
					...
					if(placement != null && (tePlacingIn == null || (tePlacingIn instanceof IMultipartTile
						&& ((MicroblockCoverSystem)((IMultipartTile)tePlacingIn).getCoverSystem()).canPlace(type, placement))))
						MicroblockRenderer.renderPartPreview(evt.context, placement, type);
